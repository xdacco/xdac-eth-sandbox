...
contract('XdacTokenCrowdsale', function ([owner, wallet, investor, investor2]) {
  ...
  it('should allow refunds', async function () {
    ...
    let contributor = await this.crowdsale.getContributorValues(investor)
    contributor[0].should.be.bignumber.equal(ether(0.01))
    await this.crowdsale.refundTokens({ from: investor, gasPrice: 3000 }).should.be.fulfilled;
  ...
  //   await this.crowdsale.buyTokens(investor, { value: goals[0].minus(ether(0.01)), from: investor }).should.be.fulfilled;
  //   let rate = await this.crowdsale.getCurrentRate()
  //   rate.should.be.bignumber.equal(rates[0])
  // })
  //
  // it('check round 1 rate', async function () {
  //   await this.crowdsale.buyTokens(investor, { value: goals[0].plus(ether(0.01)), from: investor }).should.be.fulfilled;
  //   let rate = await this.crowdsale.getCurrentRate()
  //   rate.should.be.bignumber.equal(rates[1])
  // })
  // it('check round 2 rate', async function () {
  //   await this.crowdsale.buyTokens(investor, { value: goals[1].plus(ether(0.01)), from: investor }).should.be.fulfilled;
  //   let rate = await this.crowdsale.getCurrentRate()
  //   rate.should.be.bignumber.equal(rates[2])
  // })
  // it('check round 3 rate', async function () {
  //   await this.crowdsale.buyTokens(investor, { value: goals[2].plus(ether(0.01)), from: investor }).should.be.fulfilled;
  //   let rate = await this.crowdsale.getCurrentRate()
  //   rate.should.be.bignumber.equal(rates[3])
  // })
  // it('check round 4 rate', async function () {
  //   await this.crowdsale.buyTokens(investor, { value: goals[3].plus(ether(0.01)), from: investor }).should.be.fulfilled;
  //   let rate = await this.crowdsale.getCurrentRate()
